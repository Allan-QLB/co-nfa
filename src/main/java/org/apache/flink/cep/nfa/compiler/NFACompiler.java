...
public class NFACompiler {
    ...
    static class NFAFactoryCompiler<T> {
        ...
        private State<T> createSingletonState(
            ...
            final State<T> sink = copyWithoutTransitiveNots(sinkState);
            // todo Or pattern
            if (currentPattern instanceof OrPattern) {
                final List<GroupPattern<T, ? extends T>> patterns = ((OrPattern<T, ?>) currentPattern).getPatterns();
                for (GroupPattern<T, ? extends T> pattern : patterns) {
                    singletonState.addProceed(createGroupPatternState(pattern, sinkState, proceedState, isOptional), BooleanConditions.trueFunction());
                }
            } else {
